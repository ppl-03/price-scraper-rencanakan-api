...
class TestDepoBangunanScheduler(unittest.TestCase):
    ...
    def test_run_with_search_keyword(self):
        ...
        with patch.object(scheduler, 'create_scraper') as mock_scraper:
            ...
            with patch.object(scheduler, 'load_db_service') as mock_db:
                ...
                self.assertEqual(result['vendors']['depobangunan']['scrape_attempts'], 1)
    ...
    def test_run_with_custom_vendors(self):
        ...
        with patch.object(scheduler, 'create_scraper') as mock_scraper:
            ...
            with patch.object(scheduler, 'load_db_service', return_value=None):
                result = scheduler.run(vendors=['depobangunan'], search_keyword='besi')
    ...
    def test_run_single_scrape_attempt(self):
        ...
        with patch.object(scheduler, 'create_scraper') as mock_scraper:
            ...
            with patch.object(scheduler, 'load_db_service') as mock_db:
                ...
                result = scheduler.run(vendors=['depobangunan'], search_keyword='paku')
    ...
    def test_run_products_saved(self):
        ...
        with patch.object(scheduler, 'create_scraper') as mock_scraper:
            ...
            with patch.object(scheduler, 'load_db_service') as mock_db:
                ...
                result = scheduler.run(vendors=['depobangunan'], search_keyword='cat')
    ...
    def test_run_scraper_creation_fails(self):
        ...
        with patch.object(scheduler, 'create_scraper', side_effect=Exception('Scraper error')):
            result = scheduler.run(vendors=['depobangunan'], search_keyword='test')
    ...
    def test_run_with_use_price_update(self):
        """Test running scheduler with use_price_update enabled"""
        from api.depobangunan.scheduler import DepoBangunanScheduler
        scheduler = DepoBangunanScheduler()
        ...
        with patch.object(scheduler, 'create_scraper') as mock_scraper:
            ...
            with patch.object(scheduler, 'load_db_service') as mock_db:
                mock_db.return_value.save_with_price_update.return_value = {'inserted': 1}
                
                result = scheduler.run(vendors=['depobangunan'], search_keyword='test', use_price_update=True)
                self.assertIn('depobangunan', result['vendors'])

    def test_run_with_max_products_limit(self):
        """Test running scheduler with max_products_per_keyword limit"""
        from api.depobangunan.scheduler import DepoBangunanScheduler
        scheduler = DepoBangunanScheduler()
        ...
        with patch.object(scheduler, 'create_scraper') as mock_scraper:
            ...
            with patch.object(scheduler, 'load_db_service') as mock_db:
                ...
                result = scheduler.run(vendors=['depobangunan'], search_keyword='test', max_products_per_keyword=5)
                # Should save only 5 products despite having 10
                self.assertEqual(result['vendors']['depobangunan']['saved'], 5)
