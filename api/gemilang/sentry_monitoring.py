...
class GemilangSentryMonitor:
    """Custom Sentry monitoring for Gemilang scraper operations."""
    ...
    def set_scraping_context(keyword: str, page: int = 0, additional_data: Optional[Dict] = None):
        ...
        context = {
            "keyword": keyword,
            "page": page,
            "vendor": GemilangSentryMonitor.VENDOR,
            "timestamp": time.time()
        }
        ...
        sentry_sdk.set_tag("vendor", GemilangSentryMonitor.VENDOR)
    ...
    def track_scraping_result(result: Dict[str, Any]):
        ...
        if success:
            capture_message(
                f"Gemilang scraping completed: {products_count} products found",
                level="info"
            )
        else:
            capture_message(
                f"Gemilang scraping failed with {errors_count} errors",
                level="warning"
            )


def monitor_gemilang_function(operation_name: str, component: str = GemilangSentryMonitor.COMPONENT_SCRAPER):
    """
    Decorator to monitor individual Gemilang functions.
    
    Usage:
        @monitor_gemilang_function("scrape_products", "scraper")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            GemilangSentryMonitor.add_breadcrumb(
                ...
                category=f"gemilang.{component}",
                level="info",
                data={"function": func.__name__, "args_count": len(args)}
            )
            
            # Start performance tracking
            with start_span(op=f"gemilang.{component}", description=operation_name) as span:
                # Add tags
                span.set_tag("vendor", "gemilang")
...
def track_gemilang_transaction(transaction_name: str):
    """
    Context manager to track a complete Gemilang transaction.
    
    Usage:
        with track_gemilang_transaction("scrape_gemilang_products"):
            # scraping code
            pass
    """
    transaction = start_transaction(
        op="scraping.gemilang",
        name=transaction_name
    )
    # Set tags after transaction is created
    if transaction:
        transaction.set_tag("vendor", "gemilang")
        transaction.set_tag("transaction_type", "scraping")
    return transaction


class GemilangTaskMonitor:
    ...
    def complete(self, success: bool = True, result_data: Optional[Dict] = None):
        ...
        capture_message(
            f"Gemilang task {self.task_id} completed in {execution_time:.2f}s",
